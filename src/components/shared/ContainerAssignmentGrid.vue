  ...
  >
    ...
    <div v-else class="container-grid">
      ...
      <template v-if="variant === 'form'">
        ...
        >
          <div class="container-square-card">
            <div class="square-icon-wrapper" :style="{ 
              borderColor: container.color,
              backgroundColor: isContainerSelected(container.id) ? container.color + '15' : 'rgba(255, 255, 255, 0.9)'
            }">
              <svg 
                class="coffee-beans-icon-large" 
            ...
            <span class="square-label">{{ container.name }}</span>
...
/* Updated Square Card Design - Fixed sizing */
.container-chip {
  display: flex;
  ...
  /* Remove transform to keep consistent sizing */
}

.container-chip::before {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
...
.container-chip:hover::before,
.container-chip.assigned::before {
  transform: scaleX(1);
}

.container-chip:hover:not(:disabled) {
  border-color: var(--container-color);
  /* Removed translateY to keep same size */
...
/* Fixed icon sizing to match the Green container */
.container-icon {
  width: 48px;
  height: 48px;
  border-radius: 50%;
...
/* Icon hover effects - removed scale to keep consistent size */
.container-chip:hover .coffee-beans-icon,
.container-filter-tag:hover .coffee-beans-icon {
  /* Removed transform: scale(1.1) to keep consistent icon size */
}
